...
namespace BeacomStair
{
    ...
    internal sealed class StairBuilder
    {
        ...
        private void CreateCoordinateSystem(Point startPoint, Point directionPoint)
        {
            ...
            // The first pick is the centreline of the stair foot at ground level.
        ...
        private void CreatePlatform()
        {
            ...
            double platformStartX = StairSettings.FlightRun;
            double platformEndX = StairSettings.OverallLength;
            double platformZ = StairSettings.TotalRise;

            // 1000 mm top platform, reached after the 3500 mm stair flight.
            CreatePlate(
                "BEACOM PLATFORM",
                StairSettings.TreadProfile,
                LocalPoint(platformStartX, -halfWidth, platformZ),
                LocalPoint(platformEndX, -halfWidth, platformZ),
                LocalPoint(platformEndX, halfWidth, platformZ),
                LocalPoint(platformStartX, halfWidth, platformZ),
                "3");

            // Side supports below the platform.
            double beamZ = platformZ - StairSettings.PlatformBeamReferenceDrop;

            CreateBeam(
                "BEACOM PLATFORM STRINGER L",
                StairSettings.StringerProfile,
                LocalPoint(platformStartX, -halfStringerSpacing, beamZ),
                LocalPoint(platformEndX, -halfStringerSpacing, beamZ),
                "2");
        ...
        private void CreateFlight()
        {
            ...
            // Sloping PFC stringers from the picked ground-level foot to the top platform.
            CreateBeam(
                "BEACOM FLIGHT STRINGER L",
                StairSettings.StringerProfile,
                LocalPoint(0.0, -halfStringerSpacing, 0.0),
                LocalPoint(StairSettings.FlightRun, -halfStringerSpacing, StairSettings.TotalRise),
                "2");
            ...
            // 14 horizontal treads, working from the bottom upward.
            for (int i = 0; i < StairSettings.TreadCount; i++)
            {
                double x1 = i * StairSettings.Going;
            ...
            // 15 closed risers from ground level up to platform level.
            for (int i = 0; i < StairSettings.RiseCount; i++)
            {
                double x = i * StairSettings.Going;
                double zBottom = i * rise;
                double zTop = (i + 1) * rise;

                // The top riser occurs at the end of the 3500 mm flight.
                if (i == StairSettings.RiseCount - 1)
                    x = StairSettings.FlightRun;
        ...
        private void CreateSideGuard(double y, string side)
        {
            ...
            CreateBeam(
                ...
                LocalPoint(platformStartX, y, platformZ + StairSettings.PlatformGuardHeight),
                LocalPoint(platformEndX, y, platformZ + StairSettings.PlatformGuardHeight),
                "7");

            CreateBeam(
                "BEACOM PLATFORM MID RAIL " + side,
                StairSettings.RailProfile,
                LocalPoint(platformStartX, y, platformZ + StairSettings.MidRailHeight),
                LocalPoint(platformEndX, y, platformZ + StairSettings.MidRailHeight),
                "7");

            // Three platform posts per side.
            CreateRailPost(
                platformStartX, y, platformZ,
                platformZ + StairSettings.PlatformGuardHeight, side, "PLATFORM");

            CreateRailPost(
                platformStartX + 500.0, y, platformZ,
                platformZ + StairSettings.PlatformGuardHeight, side, "PLATFORM");

            CreateRailPost(
                platformEndX, y, platformZ,
                platformZ + StairSettings.PlatformGuardHeight, side, "PLATFORM");

            // Flight rails follow the pitch line from the picked foot upward.
            CreateBeam(
                "BEACOM STAIR TOP RAIL " + side,
                StairSettings.RailProfile,
                LocalPoint(0.0, y, StairSettings.StairHandrailHeight),
                LocalPoint(
                    StairSettings.FlightRun,
                    y,
            ...
            for (double x = 0.0; x <= StairSettings.FlightRun + 0.1; x += 500.0)
            {
            ...
            // Close the height change between stair handrail and landing guard.
            CreateBeam(
        ...
        {
            double ratio = x / StairSettings.FlightRun;
